...
int main(int argc, char** argv)
{
	...
	while (node.ok())
	{
		...
		try
		{
		        listener.waitForTransform("/tb3_1", "fake1", ros::Time(0), ros::Duration(3.0));
			listener.lookupTransform("/tb3_1", "fake1", ros::Time(0), transformfl);
		...
		try
		{
		        listener.waitForTransform("fake1", "/tb3_1", ros::Time(0), ros::Duration(3.0));
			listener.lookupTransform("fake1", "/tb3_1", ros::Time(0), transformlf);
